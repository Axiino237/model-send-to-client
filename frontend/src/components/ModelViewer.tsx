...
export default function ModelViewer({ modelUrl, modelName, companyName, clientName }: ModelViewerProps) {
  ...
  if (downloadLoading) {
    ...
    return (
      <div 
        ref={containerRef}
        className={`relative w-full rounded-2xl overflow-hidden glass-panel h-[380px] sm:h-[480px] md:h-[600px] border border-white/5 shadow-2xl flex flex-col justify-center items-center bg-radial from-slate-900 via-slate-950 to-black p-6 ${
    ...
    >
      ...
      <div className="flex-1 w-full bg-radial from-slate-900 via-slate-950 to-black relative">
        <ErrorBoundary fallback={
          ...
          <Canvas camera={{ position: [0, 0, 5], fov: 45 }} gl={{ preserveDrawingBuffer: true, logarithmicDepthBuffer: true }}>
        ...
        <div className="absolute top-3 left-3 sm:top-6 sm:left-6 pointer-events-none select-none flex flex-col gap-1 opacity-60">
          <div className="flex items-center gap-1.5 text-[10px] sm:text-xs tracking-widest text-slate-400 font-semibold uppercase">
            <Shield className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-blue-500" /> Secure Viewer
          </div>
          ...
          {clientName && (
            <div className="text-[10px] sm:text-xs font-semibold text-slate-300">
        ...
        <div className="absolute top-3 right-3 sm:top-6 sm:right-6 px-2.5 sm:px-3.5 py-1 sm:py-1.5 rounded-full glass-panel-heavy text-[10px] sm:text-xs font-semibold tracking-wider text-slate-200 uppercase border border-white/10 select-none">
          {modelName}
        </div>

        {/* Toolbar Controls Overlay */}
        <div className="absolute bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-1.5 sm:gap-3 px-2.5 sm:px-4 py-1.5 sm:py-2.5 rounded-full glass-panel border border-white/10 shadow-2xl backdrop-blur-xl z-20">
          {/* Reset Camera */}
          <button 
        ...
        {showControls && (
          <div className="absolute bottom-16 sm:bottom-20 left-1/2 -translate-x-1/2 p-3 sm:p-4 rounded-2xl glass-panel-heavy border border-white/10 min-w-[210px] sm:min-w-[240px] flex flex-col gap-2 sm:gap-3 shadow-2xl z-20 fade-in select-none">
